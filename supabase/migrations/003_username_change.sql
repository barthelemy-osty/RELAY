...
create or replace policy "users_update_own" on public.users
  for update using (auth.uid() = id and role != 'admin');

create or replace policy "users_ban" on public.users
  for update using (
    exists (
...
create or replace policy "messages_delete" on public.messages
  for delete using (
    auth.uid() = sender_id
    or exists (
      select 1 from public.users
